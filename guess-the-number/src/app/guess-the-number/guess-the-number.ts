...
export class GuessTheNumber {
  ...
  public isValidGuess(guess?: number): boolean {
    return (
      guess !== undefined && guess >= 1 && guess <= GuessTheNumber.MAX_NUMBER
    );
  }

  submitGuess(): void {
    if (this.isValidGuess(this.guessedNumber)) {
      this.feedbackMessage = `Enter a number 1 and ${GuessTheNumber.MAX_NUMBER}.`;
    }
    this.attemptsLeft--;
    this.evaluateGuess();
  }
  ...
  private endGame(isWin: Boolean): void {
    ...
    this.feedbackMessage = isWin
      ...
      : `Game over! The correct answer was ${this.secretNumber}`;
  }
